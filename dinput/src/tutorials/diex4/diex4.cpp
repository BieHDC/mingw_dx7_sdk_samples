...
BOOL CreateDInput( HWND hWnd )
{
    ...
    if( FAILED( hr = g_pDI->CreateDevice( GUID_SysKeyboard, &g_pdidKeyboard,
                                          NULL ) ) )
    {
        DisplayError( hr, (char*)"CreateDevice() failed" );
        return hr;
    }
    ...
    {
        DisplayError( hr, (char*)"SetDataFormat() failed" );
        return hr;
    }
    ...
    {
        DisplayError( hr, (char*)"SetCooperativeLevel() failed" );
        return hr;
    }
    ...
    if( FAILED( hr = g_pdidKeyboard->SetProperty( DIPROP_BUFFERSIZE,
                                                  &dipdw.diph ) ) )
    {
        DisplayError( hr, (char*)"SetProperty() failed" );
        return hr;
    }
