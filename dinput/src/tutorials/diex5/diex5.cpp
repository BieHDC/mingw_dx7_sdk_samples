...
HRESULT CreateDInput( HWND hWnd )
{
    ...
    {
        DisplayError( 0, (char*)"CreateEvent() failed" );
        return E_FAIL;
    }
    ...
    if( FAILED( hr = DirectInputCreate( hInst, DIRECTINPUT_VERSION, &g_pDI,
                                        NULL ) ) )
    {
        DisplayError( hr, (char*)"DirectInputCreate() failed");
        return hr;
    }
    ...
    if( FAILED( hr = g_pDI->EnumDevices( 0, EnumDevicesCallback, hWnd,
                                         DIEDFL_ATTACHEDONLY ) ) )
    {
        DisplayError( hr, (char*)"EnumDevices() failed");
        return hr;
    }
    ...
    if( NULL == g_pDIDevice )
    {
        DisplayError( 0, (char*)"Couldn't find a VCR control" );
        return E_FAIL;
    }
...
VOID ReadVCRData( HWND hWnd )
{
    ...
    if( SUCCEEDED(hr) && cod > 0 )
    {
        ...
        for( DWORD iod = 0; iod < cod; iod++ ) 
        {
            ...
            {
                case VCROFS_PLAY:     strObj = (char*)"Play"; break;
                case VCROFS_STOP:     strObj = (char*)"Stop"; break;
                case VCROFS_REWIND:   strObj = (char*)"FF";   break;
                case VCROFS_FFORWARD: strObj = (char*)"Rew";  break;
            }
...
int PASCAL WinMain( HINSTANCE hInst, HINSTANCE, LPSTR, int nCmdShow )
{
    ...
    WNDCLASS wc = { CS_HREDRAW | CS_VREDRAW, WndProc, 0, 0, hInst,
                    LoadIcon( hInst, (LPCSTR)IDI_APPLICATION),
    ...
    if( FAILED( g_pDIDevice->Acquire() ) )
    {
        ...
        DisplayError( E_FAIL, (char*)"Cannot acquire device" );
        DestroyDInput();
        return FALSE;
    }
