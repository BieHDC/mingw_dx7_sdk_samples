    ...
    switch( pMsg->dwType )
    {
        case DPSYS_CREATEPLAYERORGROUP:
        {
            ...
            if( p->dwPlayerType == DPPLAYERTYPE_PLAYER)
                strDisplayFormat = (char*)"Player \"%s\" created\r\n";
            else if( p->dwPlayerType == DPPLAYERTYPE_GROUP)
                strDisplayFormat = (char*)"Group \"%s\" created\r\n";
            else
                strDisplayFormat = (char*)"Unknown object \"%s\" created\r\n";

            // Get pointer to player name
            if( p->dpnName.lpszShortNameA)
                strName = p->dpnName.lpszShortNameA;
            else
                strName = (char*)"unknown";
        ...
        case DPSYS_DESTROYPLAYERORGROUP:
        {
            ...
            if( p->dwPlayerType == DPPLAYERTYPE_PLAYER )
                strDisplayFormat = (char*)"Player \"%s\" destroyed\r\n";
            else if( p->dwPlayerType == DPPLAYERTYPE_GROUP )
                strDisplayFormat = (char*)"Group \"%s\" destroyed\r\n";
            else
                strDisplayFormat = (char*)"Unknown object \"%s\" destroyed\r\n";
        ...
        case DPSYS_DELETEPLAYERFROMGROUP:
        {
            ...
            if( pMsg->dwType == DPSYS_ADDPLAYERTOGROUP )
            {
                ...
                strDisplayFormat = (char*)"Player \"%s\" added to group \"%s\"\r\n";
            }
            else
            {
                dpidPlayer = pDeleteMsg->dpIdPlayer;
                dpidGroup  = pDeleteMsg->dpIdGroup;
                strDisplayFormat = (char*)"Player \"%s\" removed from group \"%s\"\r\n";
            }
        ...
        case DPSYS_DELETEGROUPFROMGROUP:
        {
            ...
            if( pMsg->dwType == DPSYS_ADDGROUPTOGROUP )
            {
                ...
                strDisplayFormat = (char*)"Group \"%s\" added to group \"%s\"\r\n";
            }
            else
            {
                dpidParentGroup = pDeleteMsg->dpIdParentGroup;
                dpidGroup = pDeleteMsg->dpIdGroup;
                strDisplayFormat = (char*)"Group \"%s\" removed from group \"%s\"\r\n";
            }
            ...
            if( (pParentGroupName) && (pParentGroupName->lpszShortNameA) )
                ...
                strParentGroupName = (char*)"unknown";
        ...
        case DPSYS_SESSIONLOST:
        {
            ...
            LPSTR strDisplayFormat = (char*)"Session lost.\r\n";
        ...
        case DPSYS_HOST:
        {
            ...
            LPSTR       strDisplayFormat = (char*)"You have become the host\r\n";
        ...
        case DPSYS_SETGROUPOWNER:
        {
            ...
            LPSTR   strDisplayFormat = (char*)"Owner changed for group \"%s\" to \"%s\" from \"%s\"\r\n";
            HRESULT hr;
...
HRESULT InitializeLobby( HWND hWnd, DPLAYINFO* pDPInfo )
{
    ...
    {   
        ...
        name.lpszShortNameA = (char*)"Golf Shack";
        hr = pDPInfo->pDP->CreateGroup( &dpID, &name, NULL, 0, 0 );
        if( FAILED(hr) )
            return hr;

        name.lpszShortNameA = (char*)"Monster Truck Rally";
        hr = pDPInfo->pDP->CreateGroup( &dpID, &name, NULL, 0, 0 );
        if( FAILED(hr) )
            return hr;

        name.lpszShortNameA = (char*)"Club Hellbender";
        hr = pDPInfo->pDP->CreateGroup( &dpID, &name, NULL, 0, 0 );
        if( FAILED(hr) )
            return hr;
    }
                       ...
                       LPSTR* pstrStr )
{
    ...
    LPSTR   strStr = NULL;
    LPSTR   strPlayerName;
    LPSTR   strDisplayFormat = (char*)"%s>\t%s\r\n";
    HRESULT hr;
...
VOID HandleStartSession( DPLCONNECTION* pConn, DPLAYINFO* pDPInfo )
{
    ...
    if( hEvent )
    {
        ...
        if( FAILED(hr) )
        {
            ErrorBox( (char*)"Could not launch application because of error %s", hr );
        }
        else
        {
