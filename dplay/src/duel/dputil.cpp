...
VOID CheckCaps()
{
    ...
    {
        TRACE(_T((char*)"Capabilities supported: Async %s %s %s\n"),
        ...
        TRACE(_T((char*)"CheckCaps - Async not supported\n"));
}
...
HRESULT DPUtil_GetPlayerLocalData( DPID pid, VOID* pData, DWORD* pdwDataSize )
{
    ...
    if( FAILED(hr) )
        TRACE( TEXT((char*)"Get Player local data failed for id %d\n"), pid );
...
HRESULT DPUtil_SetPlayerLocalData( DPID pid, VOID* pData, DWORD dwSize )
{
    ...
    if( FAILED(hr) )
        TRACE( TEXT((char*)"Set Player local data failed for id %d\n"), pid );
