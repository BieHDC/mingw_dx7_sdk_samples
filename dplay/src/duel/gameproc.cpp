    ...
    switch( pMsg->dwType )
    {
        ...
        // Async send status
        {
            ...
            if( FAILED(pComplete->hr) )
            {
                ...
                switch( pComplete->hr )
                {
                    case DPERR_CANCELLED:   strErr = _T((char*)"Cancelled"); break;
                    case DPERR_ABORTED:     strErr = _T((char*)"Aborted");   break;
                    case DPERR_TIMEOUT:     strErr = _T((char*)"Timed out"); break;
                    case DPERR_GENERIC:     strErr = _T((char*)"Generic");   break;
                    default:                strErr = _T((char*)"Unknown");
                }
                TRACE(_T((char*)"Async msg %u to %u transmission failed. err = %s %#08X\n"),
...
{        
    ...
    switch( pMsg->byType )
    {
        ...
        default:
        {
            TRACE( TEXT((char*)"Unknown message type %d\n"), pMsg->byType );
            break;
        }
    }
}
