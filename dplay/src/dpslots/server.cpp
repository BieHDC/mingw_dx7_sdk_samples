...
FAILURE:
    ...
    if( FAILED(hr) )
    {
        LogRequest( (char*)"Balance request for \"%s\" failed: 0x%08X\r\n", pAccountDesc, hr );
        LogRequest( (char*)"You need to manually create an account "
                    "in the database file.\r\n", pAccountDesc, 0 );
    }
    else
    {
        LogRequest( (char*)"Balance request for \"%s\" returned $%d\r\n", pAccountDesc, Msg.dwBalance );
    }
...
FAILURE:
    ...
    if( FAILED(hr) )
        LogRequest( (char*)"Spin request for \"%s\" failed: 0x%08X\r\n", pAccountDesc, hr );
    else
        LogRequest( (char*)"Spin request for \"%s\" returned $%d\r\n", pAccountDesc, Msg.dwAmountWonOrLost);
