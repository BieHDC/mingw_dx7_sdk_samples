                              ...
                              LPARAM lParam )
{
    ...
    SETUP_FAILURE:
            ErrorBox((char*)"Could not create DirectPlay object because of error %s", hr);
            ...
            if( idTimer )
            {
                ...
                if( FAILED(hr) && hr != DPERR_CONNECTING )
                {
                    ...
                    if( hr != DPERR_USERCANCEL )
                        ErrorBox((char*)"Enumerating sessions has failed; error %s", hr);
                }
            }
            break;
        ...
        case WM_COMMAND:
            switch(LOWORD(wParam))
            {
                case IDC_SPCOMBO:
                    ...
                    SP_FAILURE:
                        if( hr != DPERR_USERCANCEL )
                            ErrorBox((char*)"Could not select service provider because of error %s", hr);
                        break;
                    ...
                    HOST_FAILURE:
                        ErrorBox((char*)"Could not host session because of error %s", hr);
                        break;
                    ...
                    JOIN_FAILURE:
                        ErrorBox((char*)"Could not join session because of error %s", hr);
                        break;
