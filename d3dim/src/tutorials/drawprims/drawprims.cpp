...
typedef struct _d3dmc {
	D3DVALUE v[4][4];
} d3dmc;

D3DMATRIX multmat(D3DMATRIX Ain, D3DMATRIX Bin) {
	const int N = 4;
	d3dmc A = *((d3dmc*) &Ain);
	d3dmc B = *((d3dmc*) &Bin);
    d3dmc C;
                           ...
                           D3DTLVERTEX* pvVertices, DWORD dwNumVertices )
{
    ...
    //D3DMATRIX matSet = matWorld * matView * matProj; //orig
    D3DMATRIX matSet = multmat(matWorld, multmat(matView, matProj));
