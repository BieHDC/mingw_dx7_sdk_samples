    ...
    switch (iMsg)
    {
        ...
        case WM_DISPLAYCHANGE:
            ...
            if (CheckOverlaySupport() == FALSE)
                DisplayError((char*)"You have changed your adapter settings such that you no longer support this overlay.");
            return 0L;
        ...
        PSTR szCmdLine, int iCmdShow)
{
    ...
    {
        DestroyDDraw();
        DisplayError((char*)"Unable to initialize the DirectDraw object");
        return FALSE;
    }
    ...
    {
        DestroyDDraw();
        DisplayError((char*)"Your display adapter and/or driver do not support overlays.");
        return FALSE;
    }
    ...
    if (DDOverlayInit() == FALSE)
    {
        DestroyOverlay();
        DestroyDDraw();
        DisplayError((char*)"Could not create the overlay surface.  The display card may not support the requested format, or may not have enough memory available.");
        return FALSE;
    }
    ...
    if (DrawOverlay() == FALSE)
    {
        DestroyOverlay();
        DestroyDDraw();
        DisplayError((char*)"Could not draw the bitmap on the overlay surface...");
        return FALSE;
    }
